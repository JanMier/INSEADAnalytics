...
1. In the dataSet1.R file the tickers are set to ("SPY", "AAPL") so that is where we are setting up what we want to download. The actual place where the data is being downloaded is in the for loop that starts on line 20 of the same file.
2. Data for Apple:
  + The cumulative returns of the S&P index during this period is `r round(100*sum(StockReturns[,2]),1)`%.
...
3. Data for Yahoo:
  + The cumulative returns of the S&P index during this period is `r round(100*sum(StockReturns[,3]),1)`%.
  + The average daily returns of the S&P index during this period is `r round(100*mean(StockReturns[,3]),3)`%.
  + The standard deviation of the daily returns of the S&P index during this period is `r round(100*sd(StockReturns[,3]),3)`%.
...
1. This will just be `nrow(transposedData)` and `ncol(transposedData)`
2. Plot:
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
...
1. Erm.. The startDate is already set to 2001-01-01 in dataSet1.R on line 9
2. The following grath shows the returns for 3 equally rated companies (Tesla, NetApp, Facebook) over the last 3 years (from 2014-01-01)
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
s = match("2014-01-02",rownames(StockReturns))
truncatedData = StockReturns[s:nrow(StockReturns),4:6]
portfolio = apply(truncatedData,1,mean)
names(portfolio) <- rownames(truncatedData)
pnl_plot(portfolio)
```
...
```{r eval = TRUE, echo=TRUE, comment=NA, warning=FALSE, message=FALSE,results='asis'}
myData <- read.csv(file = "twentydays.csv", header = TRUE, sep=",", row.names = 1)
```
...
I changed a couple of things in the myData load (sep="," and row.names=1)

1. I get a number representing the number of times that the values in the two matrices are not equal to each other. This turns out to be `r sum(myData != StockReturns[1:20,1:2])`. 
2. The table below will happen. The numbers for SPY are correct, the numbers for AAPL are not. This is because we are trying to add two matrices of different length together. What we are getting is `myData[1:20,1] + StockReturns[1:20,1]` and `myData[1:20,2] + StockReturns[21:40,1]` because we have to cycle through each column of the StockReturns extract we are using. 
```{r eval = TRUE, echo=FALSE}
myData + StockReturns[1:40,1:2]
```
